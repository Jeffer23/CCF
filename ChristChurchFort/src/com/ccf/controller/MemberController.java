...
public class MemberController extends Application {
	...
	void initialize() {
		...
		this.memberDob.setDateFormat(ProjectProperties.sdf);
		this.livedTill.setDateFormat(ProjectProperties.sdf);
		this.marriageDate.setDateFormat(ProjectProperties.sdf);
		
		memberDob.setSelectedDate(new Date());

		livedTill.selectedDateProperty().addListener(
				new InvalidationListener() {
					public void invalidated(Observable observable) {
						System.out
								.println("livedTill set date method Starts...");
	...
	public void initData(Member member) {
		...
		try {
			...
			subscriptionAmount.setText(String.valueOf(member
			...
			if (member.getMarriageDate() == null
					|| member.getMarriageDate() == "")
				marriageDate.setSelectedDate(null);
			else
				marriageDate
						.setSelectedDate(sdf.parse(member.getMarriageDate()));
	...
	public void saveMember() {
		...
		try {
			...
			float subscriptionAmount = Float.parseFloat(this.subscriptionAmount
					.getText());
			...
			if (familyMember != null) {
				...
				controller.message.setText(member.getName()
			...
			// Clearing familyMember, This will be later set when edit/Delete
			// menu is selected.
			familyMember = null;
	...
	public void cancelAddMember() {
		...
		// Clearing familyMember, This will be later set when edit/Delete menu
		// is selected.
		familyMember = null;
